...
@Path("stats/weekly")
public class WeekResource extends ContextResource
{
	@GET
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public WeeklyStats getWeekly()
		throws IndexOutOfBoundsException, SQLException
	{
			 ...
			 DSLContext context = Database.getContext(conn))
		{
			...
			result.setLowestTemp(context.select(AGGREGATED.DATE.as("date"), AGGREGATED.MIN_AMBIENT_TEMP.as("value"))
										.from(AGGREGATED)
										.where(condition)
										.and(AGGREGATED.MIN_AMBIENT_TEMP.eq(DSL.select(DSL.max(AGGREGATED.MIN_AMBIENT_TEMP)).from(AGGREGATED).where(condition)))
										.fetchAnyInto(WeeklyStats.Day.class));
